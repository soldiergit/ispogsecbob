    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <br v-if="applyAuditVisible(scope.row.projectInfoEntity)"/>
          <el-button v-if="applyAuditVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="applyAuditHandle(scope.row.projectInfoEntity.projectId)">大创审批通过</el-button>
          <el-button v-if="applyAuditVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="retreatHandle(scope.row.projectInfoEntity, 'project_audit_apply_status')">大创审批不通过</el-button>
          <br v-if="applyBaseIsVisible(scope.row.projectInfoEntity)"/>
          <el-button v-if="applyBaseIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="applyBaseHandle(scope.row.projectInfoEntity.projectId)">基地审批通过</el-button>
          <el-button v-if="applyBaseIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="retreatHandle(scope.row.projectInfoEntity, 'project_base_apply_status')">基地审批不通过</el-button>
          <br v-if="applyMatchIsVisible(scope.row.projectInfoEntity)"/>
          <el-button v-if="applyMatchIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="applyMatchHandle(scope.row.projectInfoEntity.projectId)">比赛审批通过</el-button>
          <el-button v-if="applyMatchIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="retreatHandle(scope.row.projectInfoEntity, 'project_match_apply_status')">比赛审批不通过</el-button>
          <br v-if="applyFinishIsVisible(scope.row.projectInfoEntity)"/>
          <el-button v-if="applyFinishIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="applyFinishHandle(scope.row.projectInfoEntity.projectId)">结题审批通过</el-button>
          <el-button v-if="applyFinishIsVisible(scope.row.projectInfoEntity)" type="text" size="small" @click="retreatHandle(scope.row.projectInfoEntity, 'project_finish_apply_status')">结题审批不通过</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  ...
  import RetreatAddOrUpdate from './retreat-add-or-update'
  ...
  export default {
    ...
    methods: {
        ...
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:apply:base')) {
            if (item.projectBaseApplyStatus !== null || item.projectBaseApplyStatus !== '') {
              if (item.projectBaseApplyStatus === 1 && item.baseNoPass === 0) {
                let roleIdList = this.$store.state.user.roleIdList
                for (let roleIndex = 0; roleIndex < roleIdList.length; roleIndex++) {
                  if (roleIdList[roleIndex] === 3) {
                    return true
        ...
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:apply:match')) {
            if (item.projectMatchApplyStatus !== null || item.projectMatchApplyStatus !== '') {
              if (item.projectMatchApplyStatus === 1) {
                let roleIdList = this.$store.state.user.roleIdList
                for (let roleIndex = 0; roleIndex < roleIdList.length; roleIndex++) {
                  if (roleIdList[roleIndex] === 3) {
                    return true
        ...
        if (item.projectStatus === 0 || item.projectStatus === 1) {
          if (this.isAuth('innovate:project:apply:finish')) {
            if (item.projectFinishApplyStatus !== null || item.projectFinishApplyStatus !== '') {
              if (item.projectFinishApplyStatus === 1) {
                let roleIdList = this.$store.state.user.roleIdList
                for (let roleIndex = 0; roleIndex < roleIdList.length; roleIndex++) {
                  if (roleIdList[roleIndex] === 3) {
                    return true
      ...
      // 不通过
      retreatHandle (item, apply) {
        this.retreatVisible = true
        this.$nextTick(() => {
          switch (apply) {
            case 'project_audit_apply_status': {
              this.$refs.retreat.init(item.projectId, apply, item.projectAuditApplyStatus)
              break
            }
            case 'project_base_apply_status': {
              this.$refs.retreat.init(item.projectId, apply, item.projectBaseApplyStatus)
              break
            }
            case 'project_match_apply_status': {
              this.$refs.retreat.init(item.projectId, apply, item.projectMatchApplyStatus)
              break
            }
            case 'project_finish_apply_status': {
              this.$refs.retreat.init(item.projectId, apply, item.projectFinishApplyStatus)
              break
            }
          }
        })
